...
class AI1EC_Fixes {
  ...
  public function __construct() {
    ...
    $this->aFieldToPlaceholders = array(
      ...
      'address'                      => array(
        'header-address',
        'header-location_replacement',
        'header-only_if_empty',
        'header-examples',
        'header-info-address',
        'address',
        'address_id',
        'address_value',
        'address_only_if_empty_value',
        'checked',
        'examples',
        'row-class'
      ),
      'contact_name'                 => array(
        ...
        'header-only_if_empty',
        'header-examples',
        'header-info-contact_name',
        'contact_name',
        'contact_name_id',
        'contact_name_value',
        'contact_name_only_if_empty_value',
        'checked',
        'examples',
        'row-class'
      ),
      'location-metabox'             => array(
        'location-metabox-title',
        'location-metabox-description',
        'location-metabox-value',
        'skip-event-update-from-feed-title',
        'skip-event-update-from-feed-desc',
        'skip-event-update-from-feed-checkboxes'
      ),
      ...
      'reminder'                     => array(
        ...
        'reminder-users',
        'reminder_users',
        'reminder_email-addresses',
        'reminder_email-subject',
        'reminder_email-body-wp-editor',
        'reminder_email-body-wp-editor',
      ...
      'cats-tags'                    => array(
        'label-cats-tags-users',
        'label-cats-tags-email-addresses',
        'label-cats-tags-email-subject',
        'header-general-settings',
        'header-category-settings',
        'header-tag-settings',
        'header-cats-tags_preview',
        'header-missing-cats-tags',
        'label-category-keywords',
        'label-tag-keywords',
        'label-category-name',
        'label-tag-name',
        'label-additional-term-keywords',
        ...
        'label-_none_',
        'label-enable',
        'label-resend-posts-missing-term',
        'label-show-email-preview',
        'label-show-missing-cats-tags',
        'info-festival-category',
        'info-party-category',
        'info-single-category',
        'info-cats-tags-enable',
        'info-cats-tags-resend-posts-missing-term',
        'cats-tags-users',
        'cats-tags_users',
        'cats-tags_email-addresses',
        'cats-tags_email-subject',
        'category-keywords',
        'tag-keywords',
        'cats-tags_preview',
        'missing-cats-tags',
        'cats-tags_show-email-preview',
        'cats-tags_show-missing-cats-tags',
        'options-festival-category',
        'options-party-category',
        'cats-tags_festival-category',
        'cats-tags_party-category',
        'cats-tags_single-category',
        'cats-tags_single-category-checked',
        'cats-tags_enable',
        'cats-tags_enable-checked',
        'cats-tags_resend-posts-missing-term',
        'cats-tags_resend-posts-missing-term-checked',
        'cats-tags_show-email-preview-checked',
        'cats-tags_show-missing-cats-tags-checked'
      ),
      // ''  => array(  ),
    );
  ...
  public static function ai1ecf_activate() {
    self::init();
    self::$instance->ai1ecf_maybe_add_crons();
  }
  ...
  public static function ai1ecf_deactivate() {
    self::init();
    self::$instance->ai1ecf_remove_crons();
  }

  public function ai1ecf_remove_crons() {
            ...
            foreach ( $this->aTerms[ $strTermType ] as $objTerm ) {
              ...
              foreach ( array( 'slug', 'name', 'description' ) as $attr ) {
                ...
                $strDefaultTermKeywordCheckboxesLoc .= str_replace(
                  ...
                  array(
                    $strField . "_term-default-keywords-" . $strTermType . "-" . $objTerm->slug . '-' . $attr,
                    $strField . "_term-default-keywords-" . $strTermType . "-" . $objTerm->slug,
                    $strChecked,
                    $objTerm->$attr,
                    $objTerm->$attr
                  ),
              ...
              $aPlaceholderValues[ $strPlaceholder ] .= str_replace(
                array(
                  ...
                  '%%label-term-name%%',
                  '%%label-additional-term-keywords%%',
                  '%%label-default-term-keywords%%',
                  '%%default-term-keyword-checkboxes%%',
                  '%%value%%'
                ),
                array(
                  $strField . '_',
                  ...
                  $strTermType,
                  $objTerm->name,
                  $aPlaceholderValues['label-additional-term-keywords'],
                  $aPlaceholderValues['label-default-term-keywords'],
                  $strDefaultTermKeywordCheckboxesLoc,
                  $strAdditionalTermKeywordsValue
                ),
  ...
  public function ai1ecf_add_missing_categories_and_tags( $bNoEmailSend = false, $bOnlyMissing = false ) {
    ...
    if ( ! isset( $aOptionValues[ $strOptionNameFestivalCategory ] ) ) {
      $aOptionValues[ $strOptionNameFestivalCategory ] = $this->strOptionValueNone;
    }
    if ( ! isset( $aOptionValues[ $strOptionNamePartyCategory ] ) ) {
      $aOptionValues[ $strOptionNamePartyCategory ] = $this->strOptionValueNone;
    }
    ...
    foreach ( $aEventsWithoutCategory as $objEventPost ) {
      ...
      if ( $aOptionValues[ $strOptionNameFestivalCategory ] != $this->strOptionValueNone ) {
        if ( $iTimeEnd - $iTimeStart >= ( 24 * 3600 ) ) {
          ...
          $bTermMatched                                                                               = true;
          if ( $aOptionValues[ $strOptionNameSingleCategory ] ) {
            $bCatMatched = true;
          } // A category was matched already //
        }
      }
    ...
    if ( ! $bRes ) {
      $this->ai1ecf_add_debug_log( var_export( $bRes, true ), false, 'debug-send-notifications-res.kk' );
      $this->ai1ecf_add_debug_log( var_export( $aEmails, true ), false, 'debug-send-notifications-emails.kk' );
      $this->ai1ecf_add_debug_log( var_export( $aOptionValues, true ), false, 'debug-send-notifications-options.kk' );
      $strReportFile = __DIR__ . '/automatic-term-assignment-report-' . date( 'Ymd-Hi' ) . '.html';
      file_put_contents( $strReportFile, $strEmailBody );
    }
  }
